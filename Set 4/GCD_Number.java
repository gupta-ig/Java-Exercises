...
public class GCD_Number {
    ...
    public int calculateGCD() {
        ...
        for(int i = small; i > 0; i--) {
            boolean result = (number1%i == 0) && (number2%i == 0);
            if(result) {
                return i;
            }
            i--;
    ...
    public static void main(String[] args) {
        ...
        GCD_Number gcd = new GCD_Number(num1, num2);

        int result = gcd.calculateGCD();
        System.out.printf("LCM of %d and %d is %d", gcd.getNumber1(), gcd.getNumber2(), result).println();
        sc.close();
    }
}